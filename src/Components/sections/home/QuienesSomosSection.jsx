...
const QuienesSomosSection = ({ imagen }) => {
    ...
    return (
        <Grid width={matches ? "100%" : "90%"} mx={"auto"} container my={matches ? 5 : 25} columns={16} maxWidth={"1400px"}>
            <Grid display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"center"} item xs={matches ? 16 : 8}>
                {matches && <Typography alignSelf={"flex-start"} pl={2} variant='h4' fontSize={"20px"} fontWeight={"bold"} mb={3} color={"primary"}>¿Quiénes somos?</Typography>}
            ...
            <Grid item xs={8}>
                <Box display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"center"}>
                    {!matches &&
                        <>
                            <Typography variant='h4' fontSize={mobile ? "20px" : "36px"} fontWeight={"bold"} color={"primary"}>¿Quiénes somos?</Typography>
                            <Box
                                mt={2}
                                p={6}
